...
function MissionCaseColonisation ( $FleetRow ) {
	...
	$iPlanetCount = mysqli_field_seek(doquery ("SELECT count(*) FROM {{table}} WHERE `id_owner` = '". $FleetRow['fleet_owner'] ."' AND `planet_type` = '1'", 'planets'), 0);
	if ($FleetRow['fleet_mess'] == 0) {
		// Déjà, sommes nous a l'aller ??
		$iGalaxyPlace = mysqli_field_seek(doquery ("SELECT count(*) FROM {{table}} WHERE `galaxy` = '". $FleetRow['fleet_end_galaxy']."' AND `system` = '". $FleetRow['fleet_end_system']."' AND `planet` = '". $FleetRow['fleet_end_planet']."';", 'galaxy'), 0);
